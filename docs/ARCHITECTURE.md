...
| 2 — Routage & intersections *(routage A\* fait, testé, ET BRANCHÉ sur le trafic live par défaut — jamais observé en jeu)* | **Prérequis identifié en écrivant ce plan** : rien ne calculait de destination ni d'itinéraire pour un véhicule — `findLookaheadPoint` suivait juste "tout droit puis continuations" et visait le carrefour lui-même dès qu'un vrai choix de direction se présentait. **Fait** : `router.lua`, A* sur le graphe de segments (couche ⑥ Planification, section 4.1). `buildIndex` retrouve, une fois par graphe chargé, quelle jonction touche quelle extrémité de chaque segment ; `neighbors` énumère les segments atteignables en un saut, en respectant `oneWay`/`flipDirection` (nouvellement extrait par `tools/extract_road_graph.py` — n'était même pas lu avant, alors que ~la moitié des segments de west_coast_usa sont `oneWay`, souvent une seule chaussée d'une route à double sens plutôt qu'une vraie rue à sens unique) ; `findRoute` renvoie le chemin le plus court ; `planRandomRoute`/`pickRandomDestination` tirent une destination aléatoire avec repli si injoignable ; `findLookaheadPointOnRoute` suit la branche choisie à chaque carrefour au lieu de viser le carrefour lui-même. Un vrai bug piège de Lua (`cond and a or b` qui se rabat silencieusement sur `b` quand `a` vaut `false`, pas seulement `nil`) a cassé deux fois la logique de sens unique avant d'être corrigé — capturé par les tests avant tout déploiement. 15 scénarios testés unitairement (`tests/lua/test_router.lua`). **Branché sur `core.lua`** (section 8, décision explicite : mettre les changements directement sur le trafic live plutôt que d'attendre) : chaque véhicule en pilotage complet reçoit une destination aléatoire et suit vraiment son itinéraire à travers les vrais carrefours, re-planifie à l'arrivée ; repli sur l'ancien `findLookaheadPoint` (ne tourne jamais) si aucun itinéraire n'est encore prêt ; calcul plafonné à 2 A* par tick pour éviter un à-coup si beaucoup de véhicules en ont besoin en même temps ; interrupteur de secours `setRoutingEnabled(false)`. **Jamais testé en jeu** — c'est le changement le plus neuf du projet à ce stade. Reste : priorité à droite / stop / cédez-le-passage (le `priorityRule` du schéma JSON, section 4.2, reste `null` — proposer par défaut via `roadClass` puis affiner à la main), ronds-points, négociation multi-véhicules à un carrefour non signalé (FIFO façon CARLA, puis réservation de créneaux section M), tourne-à-gauche non protégé avec acceptation de créneau (dépend de la prédiction, phase 3bis), passages à niveau, gestionnaire de population/destinations réelles plutôt qu'aléatoires (4.3bis) | Un véhicule IA a une destination, calcule un chemin, et négocie correctement une intersection non signalée avec plusieurs autres IA |

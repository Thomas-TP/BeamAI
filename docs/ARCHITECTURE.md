...
| 3 — Comportement avancé *(priorité identifiée en playtest)* | Dépassement, fusion, MOBIL, clignotants, contrôles visuels, **évitement latéral d'obstacles (franchir une ligne blanche si besoin, sans jamais couper la trajectoire d'un autre véhicule)** — nécessite un contrôle latéral que le mod n'a pas encore (aujourd'hui, seul `ai.setSpeed` est piloté ; ni la trajectoire ni les voies) | Trafic fluide sur route à deux voies avec dépassements crédibles |
...
10. **Identifié, pas encore attaqué** : évitement latéral d'un obstacle (franchir la ligne blanche si besoin sans couper la route à un autre véhicule). Nécessite un contrôle latéral que le mod n'a pas — aujourd'hui `core.lua` ne pilote que `ai.setSpeed`, jamais la trajectoire. Relève de la phase 3 (MOBIL/dépassement), à concevoir proprement plutôt qu'à greffer dans l'urgence.
11. Nouveau test en jeu à faire avec `dist/beamai.zip` reconstruit (voir `README.md`) pour valider les trois corrections ci-dessus, puis attaquer la phase 2 (intersections) et le chantier de la phase 3 (dépassement/évitement).

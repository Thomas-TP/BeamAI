...
| 3 — Comportement avancé *(premier incrément posé, expérimental)* | ~~Évitement latéral d'un obstacle à l'arrêt~~ codé — `mobil.lua` (décision) + `avoidance.lua` (machine à états testée) + `ai.laneChange` natif du jeu (borne au gabarit de la route, donc pas de sortie de chaussée). Désactivé par défaut (`setAvoidanceEnabled`), pas encore testé en jeu — voir Test 4 du README. Reste : fusion, clignotants, contrôles visuels, dépassement d'un véhicule en mouvement (pas seulement un obstacle à l'arrêt) | Trafic fluide sur route à deux voies avec dépassements crédibles |

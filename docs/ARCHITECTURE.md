...
| 2 — Routage & intersections *(le module de routage est fait et testé ; pas encore branché sur le comportement des véhicules en jeu)* | **Prérequis identifié en écrivant ce plan** : rien ne calculait de destination ni d'itinéraire pour un véhicule — `findLookaheadPoint` suit juste "tout droit puis continuations" et vise le carrefour lui-même dès qu'un vrai choix de direction se présente. **Fait** : `router.lua`, A* sur le graphe de segments (couche ⑥ Planification, section 4.1). `buildIndex` retrouve, une fois par graphe chargé, quelle jonction touche quelle extrémité de chaque segment (l'info brute du JSON ne donne que la liste des segments d'un carrefour, pas quelle extrémité) ; `neighbors` énumère les segments atteignables en un saut, en respectant `oneWay`/`flipDirection` (nouvellement extrait par `tools/extract_road_graph.py` — n'était même pas lu avant, alors que ~la moitié des segments de west_coast_usa sont `oneWay`, souvent une seule chaussée d'une route à double sens plutôt qu'une vraie rue à sens unique) ; `findRoute` renvoie le chemin le plus court. Un vrai bug piège de Lua (`cond and a or b` qui se rabat silencieusement sur `b` quand `a` vaut `false`, pas seulement `nil`) a cassé deux fois cette logique de sens unique avant d'être corrigé — capturé par les tests avant tout déploiement. 8 scénarios testés unitairement (`tests/lua/test_router.lua`), y compris un cas qui refuse explicitement une route illégale à sens unique plutôt que de la proposer quand même. **Volontairement pas encore branché sur `core.lua`/`findLookaheadPoint`** : brancher le routage changerait le comportement de tout le trafic à chaque carrefour (les véhicules commenceraient à réellement tourner) pendant que le pilotage complet par défaut est encore en cours de validation en jeu — prochaine étape logique une fois ce test-là confirmé stable. Reste, une fois branché : `findLookaheadPoint` suit la branche choisie par l'itinéraire au lieu de viser le carrefour, attribution d'une destination par véhicule (gestionnaire de population, 4.3bis), priorité à droite / stop / cédez-le-passage (le `priorityRule` du schéma JSON, section 4.2, reste `null` — proposer par défaut via `roadClass` puis affiner à la main), ronds-points, négociation multi-véhicules à un carrefour non signalé (FIFO façon CARLA, puis réservation de créneaux section M), tourne-à-gauche non protégé avec acceptation de créneau (dépend de la prédiction, phase 3bis), passages à niveau | Un véhicule IA a une destination, calcule un chemin, et négocie correctement une intersection non signalée avec plusieurs autres IA |

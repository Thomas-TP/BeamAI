...
| 1 — Fondations ✅ *validé en jeu* | Extracteur, IDM, lecture des feux, personnalités de base — **premier test réussi sur West Coast, USA** : respecte le code de la route, roule à la bonne vitesse. Corrections post-playtest : anticipation des feux sur plusieurs segments (freinage tardif), filtre de cap pour ne pas confondre trafic traversant et véhicule devant (hésitations aux carrefours) | Un véhicule IA suit sa voie, s'arrête aux feux rouges, accélère/freine sans à-coups |
...
| 3 — Comportement avancé *(premier incrément posé, redesigné)* | ~~Évitement latéral via `ai.laneChange`~~ abandonné après test : sans effet visible (route active vidée par le jeu près du joueur) **et redondant** — `lua/vehicle/ai.lua` a son propre évitement latéral natif et continu (`side_avoidance`, actif dès `avoidCars='on'`, le défaut). Nouvelle approche : `mobil.lua` détecte l'obstacle proche/à l'arrêt, `avoidance.lua` gère le minutage, et `core.lua` se contente de (1) ne plus traiter l'obstacle comme un arrêt dur, (2) plafonner la vitesse à une allure prudente, (3) booster temporairement `ai.setParameters({awarenessForceCoef=...})` (confirmé réel) pour que l'évitement natif réagisse franchement. Désactivé par défaut, pas encore re-testé — voir Test 4 du README. Reste : fusion, clignotants, contrôles visuels, dépassement d'un véhicule en mouvement (pas seulement un obstacle à l'arrêt) | Trafic fluide sur route à deux voies avec dépassements crédibles |
...
12. Nouveau test en jeu à faire (redémarrage complet du jeu nécessaire après le changement de format de mod) — Test 3 pour les corrections précédentes, Test 4 pour l'esquive. Une fois validés, poursuivre la phase 3 (fusion, clignotants, dépassement d'un véhicule en mouvement) puis attaquer la phase 2 (intersections).

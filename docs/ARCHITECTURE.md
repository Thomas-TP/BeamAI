...
- Contournement intelligent d'obstacles (véhicule à l'arrêt, débris, animal)
...
| 1 — Fondations ✅ *validé en jeu, puis étendu au pilotage 100 % maison* | Extracteur, IDM, lecture des feux, personnalités de base — **premier test réussi sur West Coast, USA** : respecte le code de la route, roule à la bonne vitesse. Corrections post-playtest : anticipation des feux sur plusieurs segments (freinage tardif), filtre de cap pour ne pas confondre trafic traversant et véhicule devant (hésitations aux carrefours). **Changement de cap (section 4.4bis)** : ajout de `steeringController.lua` (pure pursuit) et `speedController.lua` (PID), pilotage direct par `input.event(...)` après `ai.setMode('disabled')` — remplace `ai.setSpeed`/évitement natif, testé uniquement hors-jeu à ce stade | Un véhicule IA suit sa voie, s'arrête aux feux rouges, accélère/freine sans à-coups, **sans dépendre d'aucune décision native de `ai.lua`** |
| 2 — Intersections | Priorité à droite, stops, cédez-le-passage, ronds-points, négociation multi-véhicules — choix de branche à un carrefour dans `roadGraph.findLookaheadPoint` (vise aujourd'hui le carrefour lui-même, pas une branche choisie) | Une intersection non signalée est négociée correctement entre plusieurs IA |
| 3 — Comportement avancé *(évitement reconstruit sur le pilotage maison, pas encore testé en jeu)* | ~~Évitement latéral via `ai.laneChange`~~ abandonné après test : sans effet visible. ~~Boost de l'évitement natif (`awarenessForceCoef`)~~ fonctionnait ("oui il esquive") mais **rejeté explicitement par choix de conception** (section 4.4bis) : rester dépendant de la logique native plafonne la qualité atteignable. **Fait** : `updateFullControlAvoidance` (core.lua) décale progressivement le point de visée du pure pursuit vers le côté confirmé dégagé (`roadGraph.isOffsetPathClear`, `roadGraph.offsetPointLateral`), avec une transition en douceur plutôt qu'un saut (`avoidance.currentOffsetMetres`) ; si aucun côté n'est dégagé, retente au tick suivant sans rien forcer. Testé unitairement uniquement. Reste : fusion, clignotants, contrôles visuels, dépassement d'un véhicule en mouvement (pas seulement un obstacle à l'arrêt) | Trafic fluide sur route à deux voies avec dépassements crédibles, entièrement piloté par notre propre contrôleur |
...
| 5 — Usagers vulnérables | Cyclistes, animaux (traités comme véhicules légers, aucun support natif à créer) | Cyclistes respectés avec distance latérale de sécurité |
| 6 — Stationnement & alternance | Recherche de place, créneau/bataille/épi, demi-tour, cycle stationnement ↔ circulation (4.3bis) | Un véhicule IA se gare, repart plus tard rejoindre le trafic, et un véhicule en circulation trouve une place et s'y installe durablement |
| 7 — Personnalités & apprentissage *(socle posé)* | ~~Profils de base (vitesse, distance de suivi, non-respect occasionnel des feux)~~ fait — `driverProfile.lua`, testé unitairement. Reste : profils continus plus riches, calibration sur replays, mode shadow | Diversité de styles de conduite perceptible |
| 8 — Localisation | Rulesets par pays, import OpenStreetMap | Une vraie ville importée se comporte selon ses propres règles |
| 9 — Échelle & polish | Perf à grande échelle, multijoueur, outillage debug, sortie publique | Mod jouable en conditions réelles |

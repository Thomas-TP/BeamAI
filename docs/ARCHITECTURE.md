...
| 1 — Fondations *(en cours)* | Extracteur ✅, IDM ✅, lecture des feux ✅ — tous les appels d'API vérifiés contre le vrai code source du jeu (lu directement sur disque), pas des suppositions. Package auto-installable prêt (`dist/beamai.zip`, aucune commande console requise). Reste : validation en jeu (cette machine ne peut pas lancer BeamNG.drive) | Un véhicule IA suit sa voie, s'arrête aux feux rouges, accélère/freine sans à-coups |
...
| 7 — Personnalités & apprentissage | Profils continus, calibration sur replays, mode shadow | Diversité de styles de conduite perceptible |
...
9. **Bloqué en attente de test en jeu** — cette machine ne peut pas lancer BeamNG.drive. Instructions et ce qu'il faut rapporter dans `README.md` (« Installer et tester »). Ce qui reste non validé est *comportemental*, pas la forme des appels d'API : conduite crédible une fois en mouvement, et `pickBestInstance` (trafficLights.lua) qui doit choisir le bon feu à un carrefour multi-directions.
10. Une fois la phase 1 validée en jeu, attaquer la phase 2 (intersections).

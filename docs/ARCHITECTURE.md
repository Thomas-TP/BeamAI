...
| 3 — Comportement avancé *(à refaire sur le pilotage maison)* | ~~Évitement latéral via `ai.laneChange`~~ abandonné après test : sans effet visible. ~~Boost de l'évitement natif (`awarenessForceCoef`)~~ fonctionnait ("oui il esquive") mais **rejeté explicitement par choix de conception** (section 4.4bis) : rester dépendant de la logique native plafonne la qualité atteignable. Nouvelle approche à construire : déplacement latéral du point de visée du pure pursuit (`steeringController`/`roadGraph.findLookaheadPoint`) piloté par `mobil.lua`/`avoidance.lua` (état/minutage déjà écrits, à rebrancher sur le pilotage maison au lieu de `ai.laneChange`). Reste : fusion, clignotants, contrôles visuels, dépassement d'un véhicule en mouvement (pas seulement un obstacle à l'arrêt) | Trafic fluide sur route à deux voies avec dépassements crédibles, entièrement piloté par notre propre contrôleur |
